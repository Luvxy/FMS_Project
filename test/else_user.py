...
def write_address(data, num):
    ...
    address_what = output_text = remove_specific_words(str(data[address]))
    pyperclip.copy(address_what)
    pyautogui.hotkey('ctrl', 'v')
    ...
    if is_adress:
        ...
        time.sleep(1)
        add_data(data[name], data[id_number], data[phone_number], data[address])
        return -1
...
#엑셀 path select
def select_file(selected_columns, sheet_name):
    global path
    global data
    global active_num
    global type_num
    ...
    path = filename
    data = get_data_from_excel(path, sheet_name, selected_columns)
    
#엑셀 불러오기
def get_data_from_excel(_path, date, selected_columns):
    global active_num
...
def restart_sign_new_user():
    ...
    # 생년월일 입력
    sex_num1, sex_num2 = str(df[id_number]).split('-')
    ...
    if is_data:
        ...
        user_num = str(df[phone_number]).split('-')
        if(user_num[0] == "010"):
        ...
        user_names = df[name]
        user_id_number = df[id_number]
        user_phone_number = df[phone_number]
        user_address = df[address]
...
def sign_new_user(user_data, date):
    ...
    user_num1, user_num2 = str(df[id_number]).split('-')
...
date_input = "123"
sheet_name = "지원 명단" #input("시트 이름: ")익산시 오산면 영성길 18
select_file(selected_columns, sheet_name)
